...
class TestNumbers(unittest.TestCase):
    ...
    def test_add_and_subtract(self):
        #Mix of adding and subtracting
        self.number.addToSum(5)
        self.number.subtractFromSum(3)
        self.assertEqual(self.number.sum, 12, "Combined operations works")

    #4.3
    def test_stringOfNumber(self):
        #String of a number
        self.assertEqual(self.number.stringOfNumber(5), "five", "the string is 5")
        #String with the wrong number
        self.assertNotEqual(self.number.stringOfNumber(7), "four", "the string is 5")
        #Type Error
        with self.assertRaises(TypeError):
            self.number.stringOfNumber(5.0)
        #Value Error
        with self.assertRaises(ValueError):
            self.number.stringOfNumber(10)
...
class TestECG(unittest.TestCase):
    ...
    def test_detect_peaks(self):
        #More than one peaks
        self.assertEqual(self.ecg.detect_peaks(self.test_signal, 0.5), [2,5],"Detect peaks at 2 and 5")
        #No peaks
        self.assertEqual(self.ecg.detect_peaks([0.1, 0.2, 0.3, 0.4], 0.1), [], "Detect no peaks")
        #One peak
        self.assertEqual(self.ecg.detect_peaks([0.1,0.6,0.1], 0.5), [1], "Dectect one peak at 1")
        #Less than 3 values
        self.assertEqual(self.ecg.detect_peaks([0.1,0.6], 0.5), [], "Dectect one peak at 1")

    #5.2
    def test_remove_baseline(self):
        #Baseline removed from signal
        ...
        #Value Error when signal empty
        with self.assertRaises(ValueError):
            self.ecg.remove_baseline([])
    ...
    def test_normalize(self):
        ...
        self.assertEqual(result, expected)
        #Value Error when signal is empty
        with self.assertRaises(ValueError):
            self.ecg.normalize([])
